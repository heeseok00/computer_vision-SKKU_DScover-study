...
## 4. 예시: VGGNet Fine-Tuning으로 Dogs vs Cats 분류

![VGG Fine-Tuning 2단계·학습 곡선 (슬라이드)](images/tf_keras_pretrained/04_vgg_finetuning_dogs_cats.png)

**목표:** 사전 학습 **VGG** 백본 위에 Dogs vs Cats **이진 분류**를 맞춘다.

**참고 자료**

- **Colab:** [VGGNET Fine-Tuning — Cats vs Dogs (Google Colab)](https://colab.research.google.com/drive/1KoeLnZa9wDypT1WQi3Uk5pMy6TLhSTSE?usp=sharing)  
- **튜토리얼:** [TensorFlow — Transfer learning and fine-tuning](https://www.tensorflow.org/tutorials/images/transfer_learning)

**2단계 절차 (슬라이드 요지)**

1. **1단계:** VGG **conv 베이스는 동결**하고, 맨 위 **Softmax(또는 이진 분류) 헤드만** 학습한다.  
2. **2단계:** **Fine-tuning** — 베이스의 **맨 위(또는 마지막) 3개 층** 정도를 **풀어(unfreeze)** 헤드와 함께 **작은 learning rate**로 추가 학습한다.

슬라이드 그래프에서는 **epoch 10** 근처를 기준으로 2단계가 시작되며(**Start Fine Tuning**), 그 이후 **검증 정확도**가 대략 **0.96** 부근으로 올라가고 **검증 loss**는 더 낮아지는 형태로 나타난다(실제 수치는 실행 환경·시드에 따라 달라질 수 있음).

---

## 한 줄 정리

**`tf.keras.applications`** 는 **검증된 CNN 백본 + ImageNet 가중치**를 한 줄로 불러오게 해 주고, **Dogs vs Cats** 같은 **소규모·다른 도메인** 문제에는 **헤드만 먼저 학습 → 상위 몇 층까지 풀어 파인튜닝** 같은 **다단계 전략**으로 성능을 다듬는 경우가 많다.
